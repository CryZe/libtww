        ...
        match self.format {
            ...
            Fmt::Rfc3339 => {
                if self.tm.tm_utcoff == 0 {
                    ...
                    let sign = if self.tm.tm_utcoff > 0 {
                        '+'
                    } else {
                        '-'
                    };
...
fn parse_type(fmt: &mut fmt::Formatter, ch: char, tm: &Tm) -> fmt::Result {
    match ch {
        'A' => {
            fmt.write_str(match tm.tm_wday {
                0 => "Sunday",
                1 => "Monday",
                2 => "Tuesday",
                3 => "Wednesday",
        ...
        'n' => fmt.write_str("\n"),
        'P' => {
            fmt.write_str(if tm.tm_hour < 12 {
                "am"
            } else {
                "pm"
            })
        ...
        'Z' => {
            fmt.write_str(if tm.tm_utcoff == 0 {
                "UTC"
            } else {
                ""
            })
