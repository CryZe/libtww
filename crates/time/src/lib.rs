...
pub fn get_time() -> Timespec {
    ...
    let (sec, nsec) = (time / NSEC_PER_SEC as u64, time % NSEC_PER_SEC as u64);
...
impl Sub for SteadyTime {
    ...
    fn sub(self, other: SteadyTime) -> Duration {
        Duration::nanoseconds((self.0 - other.0) as i64)
    }
}
...
impl Sub<Duration> for SteadyTime {
    ...
    fn sub(self, other: Duration) -> SteadyTime {
        SteadyTime(self.0 - other.num_nanoseconds().unwrap() as u64)
    }
}
...
mod tests {
    ...
    use libtww::std::sync::{Once, ONCE_INIT, Mutex, MutexGuard, LockResult};
    use libtww::std::mem;
    ...
    fn test_timespec_hash() {
        use libtww::std::hash::{Hash, Hasher};

        let c = &Timespec::new(3, 2);
        let d = &Timespec::new(2, 1);
        let e = &Timespec::new(2, 1);

        let mut hasher = ::libtww::std::hash::SipHasher::new();
        ...
        hasher = ::libtww::std::hash::SipHasher::new();
