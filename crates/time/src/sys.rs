...
mod inner {
    ...
    unsafe fn timegm(tm: *const libc::tm) -> time_t {
        use libtww::std::env::{set_var, var_os, remove_var};
        extern "C" {
            fn tzset();
        }
    ...
    pub fn time_to_local_tm(sec: i64, tm: &mut Tm) {
        unsafe {
            let sec = sec as time_t;
            let mut out = mem::zeroed();
            if musl::localtime_r(&sec, &mut out).is_err() {
    ...
    pub fn utc_tm_to_time(rust_tm: &Tm) -> i64 {
        ...
        let mut tm = rust_tm.clone();
        timegm(&mut tm) as i64
    }

    pub fn local_tm_to_time(rust_tm: &Tm) -> i64 {
        let mut tm = rust_tm.clone();
        musl::mktime(&mut tm) as i64
    }

    #[cfg(any(target_os = "macos", target_os = "ios"))]
    mod mac {
        use libc::{self, timeval, mach_timebase_info};
        use libtww::std::sync::{Once, ONCE_INIT};
        ...
        impl Add<Duration> for SteadyTime {
            ...
            fn add(self, other: Duration) -> SteadyTime {
                ...
                SteadyTime { t: (self.t as i64 + delta) as u64 }
            }
        }
    }
...
mod inner {
    ...
    impl SteadyTime {
        pub fn now() -> SteadyTime {
            ...
            unsafe {
                QueryPerformanceCounter(&mut t.t);
            }
            t
        }
    }
    ...
    impl Sub for SteadyTime {
        ...
        fn sub(self, other: SteadyTime) -> Duration {
            ...
            Duration::nanoseconds(mul_div_i64(diff, 1000000000, frequency() as i64))
        }
    }
    ...
    fn acquire_privileges() {
        use libtww::std::sync::{ONCE_INIT, Once};
        use advapi32::*;
        ...
        INIT.call_once(|| unsafe {
            ...
            let c = ::libtww::std::ffi::CString::new("SeTimeZonePrivilege").unwrap();
            call!(LookupPrivilegeValueA(0 as *const _, c.as_ptr(), &mut tkp.laa.Luid));
            tkp.tkp.PrivilegeCount = 1;
            tkp.laa.Attributes = SE_PRIVILEGE_ENABLED;
            call!(AdjustTokenPrivileges(hToken, FALSE, &mut tkp.tkp, 0, 0 as *mut _, 0 as *mut _));
        });
    }
