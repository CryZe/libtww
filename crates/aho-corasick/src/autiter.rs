use libtww::std::io::{self, BufRead};
use libtww::std::marker::PhantomData;
...
pub trait Automaton<P> {
    ...
    fn find_overlapping<'a, 's, Q: ?Sized + AsRef<[u8]>>(&'a self,
                                                         s: &'s Q)
    ...
    fn stream_find_overlapping<'a, R: io::Read>(&'a self,
                                                rdr: R)
...
#[inline(never)]
fn step_to_match<P, A: Automaton<P> + ?Sized>(aut: &A,
                                              text: &[u8],
                                              mut texti: usize,
                                              mut si: StateIdx)
...
fn skip3<P, A: Automaton<P> + ?Sized>(aut: &A, text: &[u8], at: usize) -> usize {
    ...
    let (b1, b2, b3) = (aut.start_bytes()[0], aut.start_bytes()[1], aut.start_bytes()[2]);
...
impl<'a, R: io::Read, P, A: Automaton<P> + ?Sized> Iterator for StreamMatchesOverlapping<'a,
                                                                                         R,
                                                                                         P,
                                                                                         A> {
    type Item = io::Result<Match>;
