...
use libtww::std::rc::Rc;

use syntax::{Expr, Repeater, CharClass, ClassRange, ByteClass, ByteRange, is_word_byte};
use utf8_ranges::{Utf8Range, Utf8Sequence, Utf8Sequences};

use prog::{Program, Inst, InstPtr, EmptyLook, InstSave, InstSplit, InstEmptyLook, InstChar,
...
impl Compiler {
    ...
    pub fn compile(mut self, exprs: &[Expr]) -> result::Result<Program, Error> {
    ...
    fn compile_many(mut self, exprs: &[Expr]) -> result::Result<Program, Error> {
        debug_assert!(exprs.len() > 1);
    ...
    fn compile_finish(mut self) -> result::Result<Program, Error> {
        ...
        self.compiled.capture_name_idx = Rc::new(self.capture_name_idx);
        Ok(self.compiled)
    }
    ...
    fn c(&mut self, expr: &Expr) -> Result {
        ...
        match *expr {
            ...
            Class(ref cls) => self.c_class(cls),
            ClassBytes(ref cls) => self.c_class_bytes(cls),
            ...
            StartText if self.compiled.is_reverse => self.c_empty_look(prog::EmptyLook::EndText),
            StartText => self.c_empty_look(prog::EmptyLook::StartText),
            EndText if self.compiled.is_reverse => self.c_empty_look(prog::EmptyLook::StartText),
            EndText => self.c_empty_look(prog::EmptyLook::EndText),
            WordBoundary => {
    ...
    fn c_repeat(&mut self, expr: &Expr, kind: Repeater, greedy: bool) -> Result {
        match kind {
            ...
            Repeater::Range { min, max: Some(max) } => self.c_repeat_range(expr, greedy, min, max),
        }
    }

    fn c_repeat_zero_or_one(&mut self, expr: &Expr, greedy: bool) -> Result {
    ...
    fn c_repeat_zero_or_more(&mut self, expr: &Expr, greedy: bool) -> Result {
        let split_entry = self.insts.len();
        let split = self.push_split_hole();
        let Patch { hole: hole_rep, entry: entry_rep } = try!(self.c(expr));
    ...
    fn c_repeat_one_or_more(&mut self, expr: &Expr, greedy: bool) -> Result {
        let Patch { hole: hole_rep, entry: entry_rep } = try!(self.c(expr));
    ...
    fn c_repeat_range_min_or_more(&mut self, expr: &Expr, greedy: bool, min: u32) -> Result {
    ...
    fn c_repeat_range(&mut self, expr: &Expr, greedy: bool, min: u32, max: u32) -> Result {
...
impl MaybeInst {
    fn fill(&mut self, goto: InstPtr) {
        let filled = match *self {
            ...
            _ => {
                unreachable!("not all instructions were compiled! found uncompiled instruction: \
                              {:?}",
                             self)
            }
        };
    ...
    fn fill_split(&mut self, goto1: InstPtr, goto2: InstPtr) {
        let filled = match *self {
            ...
            _ => {
                unreachable!("must be called on Split instruction, instead it was called on: {:?}",
                             self)
            }
        };
        *self = MaybeInst::Compiled(filled);
    ...
    fn unwrap(self) -> Inst {
        match self {
            ...
            _ => {
                unreachable!("must be called on a compiled instruction, instead it was called on: \
                              {:?}",
                             self)
            }
        }
...
impl<'a, 'b> CompileClass<'a, 'b> {
        ...
        where I: IntoIterator<Item = &'r Utf8Range>
    {
        ...
        let mut from_inst = ::libtww::std::usize::MAX;
        ...
        for byte_range in seq {
            ...
            if from_inst == ::libtww::std::usize::MAX {
            ...
            debug_assert!(from_inst < ::libtww::std::usize::MAX);
        }
        debug_assert!(from_inst < ::libtww::std::usize::MAX);
        Ok(Patch {
            hole: last_hole,
...
fn u32_to_usize(n: u32) -> usize {
    if (n as u64) > (::libtww::std::usize::MAX as u64) {
