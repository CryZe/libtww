...
use libtww::std::collections::hash_map;
use libtww::std::fmt;
...
impl Regex {
    ...
    ///
    /// ```rust
    /// # extern crate regex; use libtww::std::str; use regex::bytes::Regex;
    /// # fn main() {
    /// let re = Regex::new(r"'(?P<title>[^']+)'\s+\((?P<year>\d{4})\)")
    ///                .unwrap();
    ...
    /// # }
    /// ```
    pub fn splitn<'r, 't>(&'r self, text: &'t [u8], limit: usize) -> SplitsN<'r, 't> {
        SplitsN {
    ...
    /// See the documentation for `replace` for details on how to access
    /// submatches in the replacement text.
    pub fn replacen<R: Replacer>(&self, text: &[u8], limit: usize, mut rep: R) -> Vec<u8> {
...
pub struct CaptureNames<'r>(::libtww::std::slice::Iter<'r, Option<String>>);
...
pub struct Captures<'t> {
    ...
    named_groups: Rc<HashMap<String, usize>>,
}
