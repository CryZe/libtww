...
use libtww::std::cell::{RefCell, UnsafeCell};
...
pub struct Exec {
    ...
    cache: UnsafeCell<Option<ProgramCache>>,
}
...
impl ExecBuilder {
    ...
    pub fn build(self) -> Result<Exec, Error> {
        ...
        let mut nfa = try!(Compiler::new()
            .size_limit(self.options.size_limit)
            .bytes(self.bytes || parsed.bytes)
            .only_utf8(self.only_utf8)
            .compile(&parsed.exprs));
        let mut dfa = try!(Compiler::new()
            .size_limit(self.options.size_limit)
            .dfa(true)
            .only_utf8(self.only_utf8)
            .compile(&parsed.exprs));
        let mut dfa_reverse = try!(Compiler::new()
            .size_limit(self.options.size_limit)
            .dfa(true)
            .only_utf8(self.only_utf8)
            .reverse(true)
    ...
    fn shortest_match_at(&self, text: &[u8], start: usize) -> Option<usize> {
        ...
        match self.ro.match_type {
            ...
            MatchType::DfaAnchoredReverse => {
                match dfa::Fsm::reverse(&self.ro.dfa_reverse,
                                        &self.cache,
                                        true,
                                        &text[start..],
                                        text.len()) {
        ...
        match self.ro.match_type {
            MatchType::Literal(ty) => {
                self.find_literals(ty, text, start)
                    .and_then(|(s, e)| self.captures_nfa_with_match(slots, text, s, e))
            }
            MatchType::Dfa => {
                match self.find_dfa_forward(text, start) {
                    dfa::Result::Match((s, e)) => self.captures_nfa_with_match(slots, text, s, e),
                    dfa::Result::NoMatch(_) => None,
            ...
            MatchType::Nothing => None,
            MatchType::DfaMany => unreachable!("BUG: RegexSet cannot be used with captures"),
        }
    }
}
...
impl<'c> ExecNoSync<'c> {
    ...
    /// matching engine should be used.
    #[inline(always)] // reduces constant overhead
    fn find_dfa_anchored_reverse(&self, text: &[u8], start: usize) -> dfa::Result<(usize, usize)> {
        use dfa::Result::*;
        match dfa::Fsm::reverse(&self.ro.dfa_reverse,
                                &self.cache,
                                false,
    ...
    /// matching engine should be used.
    #[inline(always)] // reduces constant overhead
    fn exec_dfa_reverse_suffix(&self,
                               text: &[u8],
    ...
    fn captures_nfa_with_match(&self,
                               slots: &mut [Slot],
                               text: &[u8],
                               match_start: usize,
                               match_end: usize)
    ...
    fn exec_backtrack(&self,
                      ...
                      -> bool {
        if self.ro.nfa.uses_bytes() {
            backtrack::Bounded::exec(&self.ro.nfa,
                                     &self.cache,
                                     matches,
                                     slots,
                                     ByteInput::new(text, self.ro.nfa.only_utf8),
    ...
    /// match some text.
    pub fn many_matches_at(&self, matches: &mut [bool], text: &[u8], start: usize) -> bool {
        use self::MatchType::*;
        if !self.is_anchor_end_match(text) {
            return false;
        }
        match self.ro.match_type {
            ...
            Dfa | DfaAnchoredReverse | DfaSuffix | DfaMany => {
                match dfa::Fsm::forward_many(&self.ro.dfa, &self.cache, matches, text, start) {
                    dfa::Result::Match(_) => true,
                    dfa::Result::NoMatch(_) => false,
                    dfa::Result::Quit => {
                        self.exec_nfa(MatchNfaType::Auto, matches, &mut [], false, text, start)
                    }
                }
            }
    ...
    pub fn capture_name_idx(&self) -> &Rc<HashMap<String, usize>> {
