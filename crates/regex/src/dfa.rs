...
impl<'a> Fsm<'a> {
    ...
    #[inline(always)] // reduces constant overhead
    pub fn forward_many(prog: &'a Program,
                        cache: &ProgramCache,
                        matches: &mut [bool],
                        text: &[u8],
                        at: usize)
    ...
    /// state.
    fn cached_state_key(&mut self, q: &SparseSet, state_flags: &mut StateFlags) -> Option<State> {
        use prog::Inst::*;
    ...
    fn start_state(&mut self,
        ...
        let flagi = {
            (((empty_flags.start as u8) << 0) | ((empty_flags.end as u8) << 1) |
             ...
             ((state_flags.is_word() as u8) << 4)) as usize
        };
    ...
    fn add_state(&mut self, state: State) -> Option<StatePtr> {
        ...
        self.cache.size += self.cache.trans.state_heap_size() + (2 * state.data.len()) +
                           (2 * mem::size_of::<State>()) +
    ...
    fn has_prefix(&self) -> bool {
        !self.prog.is_reverse && !self.prog.prefixes.is_empty() && !self.prog.is_anchored_start
    }
...
fn vb(b: usize) -> String {
    ...
    if b > ::libtww::std::u8::MAX as usize {
        "EOF".to_owned()
    } else {
...
fn usize_to_u32(n: usize) -> u32 {
    if (n as u64) > (::libtww::std::u32::MAX as u64) {
...
mod tests {
    ...
    use super::{StateFlags, State, push_inst_ptr, write_varu32, read_varu32, write_vari32,
                read_vari32};
