...
pub struct Program {
    ...
    pub capture_name_idx: Rc<HashMap<String, usize>>,
...
impl Program {
    ...
    pub fn new() -> Self {
        Program {
            ...
            matches: vec![],
            captures: vec![],
            capture_name_idx: Rc::new(HashMap::new()),
            start: 0,
    ...
    pub fn approximate_size(&self) -> usize {
        ...
        (self.len() * mem::size_of::<Inst>()) + (self.matches.len() * mem::size_of::<InstPtr>()) +
        (self.captures.len() * mem::size_of::<Option<String>>()) +
        (self.capture_name_idx.len() * (mem::size_of::<String>() + mem::size_of::<usize>())) +
        (self.byte_classes.len() * mem::size_of::<u8>()) + self.prefixes.approximate_size()
    }
}
...
impl fmt::Debug for Program {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        for (pc, inst) in self.iter().enumerate() {
            match *inst {
                Match(slot) => try!(write!(f, "{:04} Match({:?})", pc, slot)),
                Save(ref inst) => {
                    let s = format!("{:04} Save({})", pc, inst.slot);
                    try!(write!(f, "{}", with_goto(pc, inst.goto, s)));
                }
                Split(ref inst) => {
                    try!(write!(f, "{:04} Split({}, {})", pc, inst.goto1, inst.goto2));
                }
                EmptyLook(ref inst) => {
                    let s = format!("{:?}", inst.look);
                    try!(write!(f, "{:04} {}", pc, with_goto(pc, inst.goto, s)));
                }
                Char(ref inst) => {
                    let s = format!("{:?}", inst.c);
                    try!(write!(f, "{:04} {}", pc, with_goto(pc, inst.goto, s)));
