...
use libtww::std::borrow::Cow;
use libtww::std::collections::HashMap;
use libtww::std::fmt;
use libtww::std::ops::Index;
use libtww::std::str::FromStr;
...
/// ```
///
/// # Using the `libtww::std::str::StrExt` methods with `Regex`
///
/// > **Note**: This section requires that this crate is currently compiled with
/// >           the `pattern` Cargo feature enabled.
///
/// Since `Regex` implements `Pattern`, you can use regexes with methods
/// defined on `libtww::std::str::StrExt`. For example, `is_match`, `find`, `find_iter`
...
impl Regex {
    ...
    pub fn find_at(&self, text: &str, start: usize) -> Option<(usize, usize)> {
        match self.0 {
            _Regex::Dynamic(ref exec) => exec.searcher_str().find_at(text, start),
    ...
    pub fn captures<'t>(&self, text: &'t str) -> Option<Captures<'t>> {
        ...
        self.read_captures_at(&mut slots, text, 0).map(|_| {
            Captures {
                text: text,
                slots: slots,
    ...
    /// match when `start == 0`.
    #[doc(hidden)]
    pub fn read_captures_at(&self,
                            slots: &mut [Slot],
                            text: &str,
                            start: usize)
                            -> Option<(usize, usize)> {
        match self.0 {
            _Regex::Dynamic(ref exec) => exec.searcher_str().read_captures_at(slots, text, start),
            _Regex::Plugin(ref plug) => plug.read_captures_at(slots, text, start),
        }
    }
    ...
    pub fn shortest_match_at(&self, text: &str, start: usize) -> Option<usize> {
        match self.0 {
            _Regex::Dynamic(ref exec) => exec.searcher_str().shortest_match_at(text, start),
    ...
    pub fn capture_names(&self) -> CaptureNames {
        CaptureNames(match self.0 {
            ...
            _Regex::Dynamic(ref d) => _CaptureNames::Dynamic(d.capture_names().iter()),
        })
    }
...
enum _CaptureNames<'r> {
    #[doc(hidden)]
    Plugin(::libtww::std::slice::Iter<'r, Option<&'static str>>),
    #[doc(hidden)]
    Dynamic(::libtww::std::slice::Iter<'r, Option<String>>),
}
...
enum NamedGroups {
    ...
    Dynamic(Rc<HashMap<String, usize>>),
}
...
impl NamedGroups {
    fn from_regex(regex: &Regex) -> NamedGroups {
        match regex.0 {
            ...
            _Regex::Dynamic(ref exec) => NamedGroups::Dynamic(exec.capture_name_idx().clone()),
        }
    }
    ...
    fn pos(&self, name: &str) -> Option<usize> {
        match *self {
            ...
            NamedGroups::Dynamic(ref groups) => groups.get(name).map(|i| *i),
        }
    }
...
enum NamedGroupsIter<'n> {
    Plugin(::libtww::std::slice::Iter<'static, (&'static str, usize)>),
    Dynamic(::libtww::std::collections::hash_map::Iter<'n, String, usize>),
}
...
impl<'n> Iterator for NamedGroupsIter<'n> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match *self {
            ...
            NamedGroupsIter::Dynamic(ref mut it) => it.next().map(|(s, i)| (s.as_ref(), *i)),
        }
    }
}
