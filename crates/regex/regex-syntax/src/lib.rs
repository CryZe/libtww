...
use libtww::std::ascii;
use libtww::std::char;
use libtww::std::cmp::{Ordering, max, min};
use libtww::std::fmt;
use libtww::std::iter::IntoIterator;
use libtww::std::ops::Deref;
use libtww::std::result;
...
impl Expr {
    ...
    pub fn is_anchored_start(&self) -> bool {
        match *self {
            Repeat { ref e, r, .. } => !r.matches_empty() && e.is_anchored_start(),
    ...
    pub fn has_anchored_start(&self) -> bool {
        match *self {
            Repeat { ref e, r, .. } => !r.matches_empty() && e.has_anchored_start(),
    ...
    pub fn has_anchored_end(&self) -> bool {
        match *self {
            Repeat { ref e, r, .. } => !r.matches_empty() && e.has_anchored_end(),
...
impl IntoIterator for CharClass {
    ...
    fn into_iter(self) -> vec::IntoIter<ClassRange> {
...
impl<'a> IntoIterator for &'a CharClass {
    ...
    fn into_iter(self) -> slice::Iter<'a, ClassRange> {
        self.iter()
    }
}
...
impl CharClass {
    ...
    pub fn negate(mut self) -> CharClass {
        fn range(s: char, e: char) -> ClassRange {
            ClassRange::new(s, e)
        }
        ...
        for win in self.windows(2) {
            inv.ranges.push(range(inc_char(win[0].end), dec_char(win[1].start)));
        }
        if self[self.len() - 1].end < char::MAX {
            inv.ranges.push(range(inc_char(self[self.len() - 1].end), char::MAX));
        }
        inv
    }
...
impl ClassRange {
    ...
    fn needs_case_folding(self) -> bool {
        case_folding::C_plus_S_both_table.binary_search_by(|&(c, _)| self.partial_cmp(&c).unwrap())
            .is_ok()
    }
...
impl ByteClass {
    ...
    pub fn negate(mut self) -> ByteClass {
        ...
        for win in self.windows(2) {
            inv.ranges.push(range(win[0].end.saturating_add(1), win[1].start.saturating_sub(1)));
        }
        if self[self.len() - 1].end < u8::MAX {
            inv.ranges.push(range(self[self.len() - 1].end.saturating_add(1), u8::MAX));
        }
        inv
    }
...
impl ByteRange {
    ...
    fn overlapping(self, other: ByteRange) -> bool {
        max(self.start, other.start) <= min(self.end, other.end).saturating_add(1)
    }
...
impl IntoIterator for ByteClass {
    ...
    fn into_iter(self) -> vec::IntoIter<ByteRange> {
        self.ranges.into_iter()
    }
}
...
impl<'a> IntoIterator for &'a ByteClass {
    ...
    fn into_iter(self) -> slice::Iter<'a, ByteRange> {
        self.iter()
    }
}
...
impl fmt::Display for Expr {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match *self {
            ...
            Group { ref e, name: Some(ref n), .. } => write!(f, "(?P<{}>{})", n, e),
...
pub type Result<T> = libtww::std::result::Result<T, Error>;
...
impl fmt::Display for ErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        use ErrorKind::*;
        match *self {
            DoubleFlagNegation => write!(f, "Only one negation symbol is allowed in flags."),
            DuplicateCaptureName(ref s) => {
                write!(f, "Capture name '{}' is used more than once.", s)
            }
            EmptyAlternate => write!(f, "Alternations cannot be empty."),
            EmptyCaptureName => write!(f, "Capture names cannot be empty."),
            EmptyFlagNegation => write!(f, "Flag negation requires setting at least one flag."),
            EmptyGroup => write!(f, "Empty regex groups (e.g., '()') are not allowed."),
            InvalidBase10(ref s) => write!(f, "Not a valid base 10 number: '{}'", s),
            InvalidBase16(ref s) => write!(f, "Not a valid base 16 number: '{}'", s),
            InvalidCaptureName(ref s) => {
                write!(f,
                       "Invalid capture name: '{}'. Capture names must consist of [_a-zA-Z0-9] \
                        and are not allowed to start with with a number.",
                       s)
            }
            InvalidClassRange { start, end } => {
                write!(f,
                       "Invalid character class range '{}-{}'. Character class ranges must start \
                        with the smaller character, but {} > {}",
                       start,
                       end,
                       start,
                       end)
            ...
            InvalidClassEscape(ref e) => {
                write!(f, "Invalid escape sequence in character class: '{}'.", e)
            }
            InvalidRepeatRange { min, max } => {
                write!(f,
                       "Invalid counted repetition range: {{{}, {}}}. Counted repetition ranges \
                        must start with the minimum, but {} > {}",
                       min,
                       max,
                       min,
                       max)
            ...
            InvalidScalarValue(c) => {
                write!(f,
                       "Number does not correspond to a Unicode scalar value: '{}'.",
                       c)
            }
            MissingBase10 => write!(f, "Missing maximum in counted reptition operator."),
            RepeaterExpectsExpr => write!(f, "Missing expression for reptition operator."),
            RepeaterUnexpectedExpr(ref e) => {
                write!(f, "Invalid application of reptition operator to: '{}'.", e)
            }
            UnclosedCaptureName(ref s) => {
                write!(f,
                       "Capture name group for '{}' is not closed. (Missing a '>'.)",
                       s)
            }
            UnclosedHex => write!(f, "Unclosed hexadecimal literal (missing a '}}')."),
            UnclosedParen => write!(f, "Unclosed parenthesis."),
            UnclosedRepeat => write!(f, "Unclosed counted repetition (missing a '}}')."),
            UnclosedUnicodeName => write!(f, "Unclosed Unicode literal (missing a '}}')."),
            UnexpectedClassEof => {
                write!(f,
                       "Character class was not closed before the end of the regex (missing a \
                        ']').")
            }
            UnexpectedEscapeEof => {
                write!(f,
                       "Started an escape sequence that didn't finish before the end of the regex.")
            }
            UnexpectedFlagEof => {
                write!(f,
                       "Inline flag settings was not closed before the end of the regex (missing \
                        a ')' or ':').")
            }
            UnexpectedTwoDigitHexEof => {
                write!(f, "Unexpected end of two digit hexadecimal literal.")
            }
            UnopenedParen => write!(f, "Unopened parenthesis."),
            UnrecognizedEscape(c) => write!(f, "Unrecognized escape sequence: '\\{}'.", c),
            UnrecognizedFlag(c) => {
                write!(f,
                       "Unrecognized flag: '{}'. (Allowed flags: i, m, s, U, u, x.)",
                       c)
            }
            ...
            StackExhausted => {
                write!(f,
                       "Exhausted space required to parse regex with too much nesting.")
            }
            FlagNotAllowed(flag) => write!(f, "Use of the flag '{}' is not allowed.", flag),
            UnicodeNotAllowed => {
                write!(f,
                       "Unicode features are not allowed when the Unicode (u) flag is not set.")
            }
            InvalidUtf8 => write!(f, "Matching arbitrary bytes is not allowed."),
            EmptyClass => write!(f, "Empty character classes are not allowed."),
            __Nonexhaustive => unreachable!(),
        }
    }
}
...
mod tests {
    ...
    fn class_canon_unordered() {
        ...
        assert_eq!(cls.canonicalize(), class(&[('a', 'c'), ('x', 'z')]));
    }

    #[test]
    fn class_canon_overlap() {
        let cls = class(&[('x', 'z'), ('w', 'y')]);
        assert_eq!(cls.canonicalize(), class(&[('w', 'z')]));
    }

    #[test]
    fn class_canon_overlap_many() {
        let cls = class(&[('c', 'f'), ('a', 'g'), ('d', 'j'), ('a', 'c'), ('m', 'p'), ('l', 's')]);
        assert_eq!(cls.clone().canonicalize(), class(&[('a', 'j'), ('l', 's')]));
    }

    #[test]
    fn class_canon_overlap_boundary() {
        let cls = class(&[('x', 'z'), ('u', 'w')]);
        assert_eq!(cls.canonicalize(), class(&[('u', 'z')]));
    }

    #[test]
    fn class_canon_extreme_edge_case() {
        let cls = class(&[('\x00', '\u{10FFFF}'), ('\x00', '\u{10FFFF}')]);
        assert_eq!(cls.canonicalize(), class(&[('\x00', '\u{10FFFF}')]));
    }
    ...
    fn class_negate_single() {
        let cls = class(&[('a', 'a')]);
        assert_eq!(cls.negate(),
                   class(&[('\x00', '\x60'), ('\x62', '\u{10FFFF}')]));
    }
    ...
    fn class_negate_multiples() {
        let cls = class(&[('a', 'c'), ('x', 'z')]);
        assert_eq!(cls.negate(),
                   class(&[('\x00', '\x60'), ('\x64', '\x77'), ('\x7b', '\u{10FFFF}')]));
    }

    #[test]
    fn class_negate_min_scalar() {
        let cls = class(&[('\x00', 'a')]);
        assert_eq!(cls.negate(), class(&[('\x62', '\u{10FFFF}')]));
    }

    #[test]
    fn class_negate_max_scalar() {
        let cls = class(&[('a', '\u{10FFFF}')]);
        assert_eq!(cls.negate(), class(&[('\x00', '\x60')]));
    }
    ...
    fn class_negate_everything_sans_one() {
        let cls = class(&[('\x00', '\u{10FFFD}'), ('\u{10FFFF}', '\u{10FFFF}')]);
        assert_eq!(cls.negate(), class(&[('\u{10FFFE}', '\u{10FFFE}')]));
    }

    #[test]
    fn class_negate_surrogates_min() {
        let cls = class(&[('\x00', '\u{D7FF}')]);
        assert_eq!(cls.negate(), class(&[('\u{E000}', '\u{10FFFF}')]));
    }

    #[test]
    fn class_negate_surrogates_min_edge() {
        let cls = class(&[('\x00', '\u{D7FE}')]);
        assert_eq!(cls.negate(), class(&[('\u{D7FF}', '\u{10FFFF}')]));
    }

    #[test]
    fn class_negate_surrogates_max() {
        let cls = class(&[('\u{E000}', '\u{10FFFF}')]);
        assert_eq!(cls.negate(), class(&[('\x00', '\u{D7FF}')]));
    }

    #[test]
    fn class_negate_surrogates_max_edge() {
        let cls = class(&[('\u{E001}', '\u{10FFFF}')]);
        assert_eq!(cls.negate(), class(&[('\x00', '\u{E000}')]));
    }
    ...
    fn class_fold_az() {
        ...
        assert_eq!(cls.case_fold(), bclass(&[(b'A', b'Z'), (b'a', b'z')]));
        let cls = bclass(&[(b'a', b'z')]);
        assert_eq!(cls.case_fold(), bclass(&[(b'A', b'Z'), (b'a', b'z')]));
    }
    ...
    fn class_fold_a_underscore() {
        ...
        assert_eq!(cls.clone().canonicalize(), class(&[('A', 'A'), ('_', '_')]));
        assert_eq!(cls.case_fold(),
                   class(&[('A', 'A'), ('_', '_'), ('a', 'a')]));

        let cls = bclass(&[(b'A', b'A'), (b'_', b'_')]);
        assert_eq!(cls.clone().canonicalize(),
                   bclass(&[(b'A', b'A'), (b'_', b'_')]));
        assert_eq!(cls.case_fold(),
                   bclass(&[(b'A', b'A'), (b'_', b'_'), (b'a', b'a')]));
    }

    #[test]
    fn class_fold_a_equals() {
        let cls = class(&[('A', 'A'), ('=', '=')]);
        assert_eq!(cls.clone().canonicalize(), class(&[('=', '='), ('A', 'A')]));
        assert_eq!(cls.case_fold(),
                   class(&[('=', '='), ('A', 'A'), ('a', 'a')]));
        ...
        assert_eq!(cls.clone().canonicalize(),
                   bclass(&[(b'=', b'='), (b'A', b'A')]));
        assert_eq!(cls.case_fold(),
                   bclass(&[(b'=', b'='), (b'A', b'A'), (b'a', b'a')]));
    }

    #[test]
    fn class_fold_no_folding_needed() {
        let cls = class(&[('\x00', '\x10')]);
        assert_eq!(cls.case_fold(), class(&[('\x00', '\x10')]));

        let cls = bclass(&[(b'\x00', b'\x10')]);
        assert_eq!(cls.case_fold(), bclass(&[(b'\x00', b'\x10')]));
    }

    #[test]
    fn class_fold_negated() {
        let cls = class(&[('x', 'x')]);
        assert_eq!(cls.clone().case_fold(), class(&[('X', 'X'), ('x', 'x')]));
        assert_eq!(cls.case_fold().negate(),
                   class(&[('\x00', 'W'), ('Y', 'w'), ('y', '\u{10FFFF}')]));

        let cls = bclass(&[(b'x', b'x')]);
        assert_eq!(cls.clone().case_fold(),
                   bclass(&[(b'X', b'X'), (b'x', b'x')]));
        assert_eq!(cls.case_fold().negate(),
                   bclass(&[(b'\x00', b'W'), (b'Y', b'w'), (b'y', b'\xff')]));
    }
    ...
    fn class_fold_single_to_multiple() {
        let cls = class(&[('k', 'k')]);
        assert_eq!(cls.case_fold(),
                   class(&[('K', 'K'), ('k', 'k'), ('\u{212A}', '\u{212A}')]));

        let cls = bclass(&[(b'k', b'k')]);
        assert_eq!(cls.case_fold(), bclass(&[(b'K', b'K'), (b'k', b'k')]));
    }
