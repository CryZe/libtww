...
use {Expr, Repeater, CharClass, ClassRange, CaptureIndex, CaptureName, Error, ErrorKind, Result};
...
impl Parser {
    ...
    fn parse_escape(&mut self) -> Result<Build> {
        ...
        match c {
            ...
            'A' => {
                self.bump();
                Ok(Build::Expr(Expr::StartText))
            }
            'z' => {
                self.bump();
                Ok(Build::Expr(Expr::EndText))
            }
            'b' => {
                self.bump();
    ...
    fn parse_group_flags(&mut self, opening_chari: usize) -> Result<Build> {
        ...
        loop {
            ...
            match self.cur() {
                'i' => {
                    self.flags.casei = sign;
                    saw_flag = true
                }
                'm' => {
                    self.flags.multi = sign;
                    saw_flag = true
                }
                's' => {
                    self.flags.dotnl = sign;
                    saw_flag = true
                }
                'U' => {
                    self.flags.swap_greed = sign;
                    saw_flag = true
                }
                'x' => {
                    self.flags.ignore_space = sign;
                    saw_flag = true
                }
                'u' => {
    ...
    fn parse_decimal<B: Bumpable>(&mut self, until: B) -> Result<u32> {
        match self.bump_get(until) {
            ...
            Some(n) => {
                ...
                // e.g., a{9999999999}
                let n = n.trim();
                u32::from_str_radix(n, 10).map_err(|_| self.err(ErrorKind::InvalidBase10(n.into())))
            }
        }
    }
    ...
    fn parse_hex_many_digits(&mut self) -> Result<Build> {
        ...
        let c = try!(char::from_u32(n).ok_or(self.err(ErrorKind::InvalidScalarValue(n))));
        Ok(try!(self.lit(c)))
    }
    ...
    // Start: `-` (with start == `a`)
    // End:   `]`
    fn parse_class_range(&mut self, class: &mut CharClass, start: char) -> Result<()> {
        if !self.bump_if('-') {
        ...
        let end = match self.cur() {
            '\\' => {
                match try!(self.parse_escape()) {
                    ...
                    Build::Expr(Expr::LiteralBytes { bytes, .. }) => bytes[0] as char,
                    Build::Expr(e) => {
...
impl Parser {
    ...
    fn pop(&mut self, expected: ErrorKind) -> Result<Expr> {
        match self.stack.pop() {
            None |
            Some(Build::LeftParen { .. }) => Err(self.err(expected)),
...
const ASCII_CLASSES: NamedClasses = &[// Classes must be in alphabetical order so that bsearch works.
...
#[cfg(test)]
mod tests {
    use {CharClass, ClassRange, ByteClass, ByteRange, Expr, Repeater, ErrorKind};
    use unicode::regex::{PERLD, PERLS, PERLW};
    use super::{LOWER, UPPER, WORD, Flags, Parser, ascii_class};

    static YI: &'static [(char, char)] = &[('\u{a000}', '\u{a48c}'), ('\u{a490}', '\u{a4c6}')];

    fn p(s: &str) -> Expr {
        Parser::parse(s, Flags::default()).unwrap()
    }
    fn pf(s: &str, flags: Flags) -> Expr {
    ...
    fn flags_inline_multiple_negate_one() {
        assert_eq!(p("(?is)a.(?i-s)a."),
                   c(&[liti('a'), Expr::AnyChar, liti('a'), Expr::AnyCharNoNL]));
    }

    #[test]
    fn any_byte() {
        assert_eq!(pb("(?-u).(?u)."),
                   c(&[Expr::AnyByteNoNL, Expr::AnyCharNoNL]));
        assert_eq!(pb("(?s)(?-u).(?u)."), c(&[Expr::AnyByte, Expr::AnyChar]));
    }
    ...
    fn escape_simple() {
        assert_eq!(p(r"\a\f\t\n\r\v"),
                   c(&[lit('\x07'), lit('\x0C'), lit('\t'), lit('\n'), lit('\r'), lit('\x0B')]));
    }

    #[test]
    fn escape_boundaries() {
        assert_eq!(p(r"\A\z\b\B"),
                   c(&[Expr::StartText, Expr::EndText, Expr::WordBoundary, Expr::NotWordBoundary]));
        assert_eq!(pb(r"(?-u)\b\B"),
                   c(&[Expr::WordBoundaryAscii, Expr::NotWordBoundaryAscii]));
    }

    #[test]
    fn escape_punctuation() {
        assert_eq!(p(r"\\\.\+\*\?\(\)\|\[\]\{\}\^\$\#"),
                   c(&[lit('\\'), lit('.'), lit('+'), lit('*'), lit('?'), lit('('), lit(')'),
    ...
    fn escape_perl_d_case_fold_negate() {
        assert_eq!(p(r"(?i)\D"), Expr::Class(class(PERLD).case_fold().negate()));
    ...
    fn escape_perl_s_case_fold_negate() {
        assert_eq!(p(r"(?i)\S"), Expr::Class(class(PERLS).case_fold().negate()));
    ...
    fn escape_perl_w_case_fold_negate() {
        assert_eq!(p(r"(?i)\W"), Expr::Class(class(PERLW).case_fold().negate()));
    ...
    fn class_singleton_negate() {
        assert_eq!(p(r"[^a]"),
                   Expr::Class(class(&[('\x00', '\x60'), ('\x62', '\u{10FFFF}')])));
        assert_eq!(p(r"[^\x00]"), Expr::Class(class(&[('\x01', '\u{10FFFF}')])));
        assert_eq!(p(r"[^\n]"),
                   Expr::Class(class(&[('\x00', '\x09'), ('\x0b', '\u{10FFFF}')])));
        assert_eq!(p("[^\n]"),
                   Expr::Class(class(&[('\x00', '\x09'), ('\x0b', '\u{10FFFF}')])));

        assert_eq!(pb(r"(?-u)[^a]"),
                   Expr::ClassBytes(bclass(&[(0x00, 0x60), (0x62, 0xFF)])));
        assert_eq!(pb(r"(?-u)[^\x00]"),
                   Expr::ClassBytes(bclass(&[(0x01, 0xFF)])));
        assert_eq!(pb(r"(?-u)[^\n]"),
                   Expr::ClassBytes(bclass(&[(0x00, 0x09), (0x0B, 0xFF)])));
        assert_eq!(pb("(?-u)[^\n]"),
                   Expr::ClassBytes(bclass(&[(0x00, 0x09), (0x0B, 0xFF)])));
    }
    ...
    fn class_multiple_class() {
        assert_eq!(p(r"[\d\p{Yi}]"), Expr::Class(classes(&[PERLD, YI])));
    }

    #[test]
    fn class_multiple_class_negate() {
        assert_eq!(p(r"[^\d\p{Yi}]"),
                   Expr::Class(classes(&[PERLD, YI]).negate()));
    }
    ...
    fn class_multiple_class_casei() {
        assert_eq!(p(r"(?i)[\d\p{Yi}]"),
                   Expr::Class(classes(&[PERLD, YI]).case_fold()));
    }

    #[test]
    fn class_multiple_class_negate_casei() {
        assert_eq!(p(r"(?i)[^\d\p{Yi}]"),
                   Expr::Class(classes(&[PERLD, YI])
                       .case_fold()
                       .negate()));
    }
    ...
    fn class_class_hypen() {
        assert_eq!(p(r"[\p{Yi}-]"), Expr::Class(classes(&[&[('-', '-')], YI])));
        assert_eq!(p(r"[\p{Yi}-a]"),
                   Expr::Class(classes(&[&[('-', '-')], &[('a', 'a')], YI])));
    }
    ...
    fn ascii_classes_not() {
        assert_eq!(p("[:abc:]"), Expr::Class(class(&[(':', ':'), ('a', 'c')])));
        assert_eq!(pb("(?-u)[:abc:]"),
    ...
    fn ascii_classes_case_fold() {
        ...
        assert_eq!(pb("(?i-u)[:upper:]"), Expr::Class(class(UPPER).case_fold()));
    ...
    fn ascii_classes_negate_case_fold() {
        ...
        assert_eq!(pb("(?i-u)[[:^upper:]]"),
                   Expr::ClassBytes(class(UPPER).to_byte_class().case_fold().negate()));
        assert_eq!(pb("(?i-u)[^[:^upper:]]"),
                   Expr::ClassBytes(class(UPPER).to_byte_class().case_fold()));
    }
...
(?-x:#) # sweet"),
    ...
    fn error_repeat_illegal_exprs_simple() {
        test_err!("a**",
                  2,
                  ErrorKind::RepeaterUnexpectedExpr(Expr::Repeat {
                      e: b(lit('a')),
                      r: Repeater::ZeroOrMore,
                      greedy: true,
                  }));
    ...
    fn error_repeat_invalid_number() {
        test_err!("a{9999999999}",
                  12,
                  ErrorKind::InvalidBase10("9999999999".into()));
        test_err!("a{1,9999999999}",
                  14,
                  ErrorKind::InvalidBase10("9999999999".into()));
    }
    ...
    fn error_class_invalid_escape() {
        test_err!(r"[\pA]", 4, ErrorKind::UnrecognizedUnicodeClass("A".into()));
    }
    ...
    fn error_class_range_valid_escape_not_allowed() {
        ...
        test_err!(r"[a-\A]", 5, ErrorKind::InvalidClassEscape(Expr::StartText));
        test_err!(r"[\A-a]", 3, ErrorKind::InvalidClassEscape(Expr::StartText));
    }
