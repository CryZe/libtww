...
impl<T> Sender<T> {
    ...
    pub fn send(&self, t: T) -> Result<(), SendError<T>> {
        let (new_inner, ret) = match *unsafe { self.inner() } {
            ...
            Flavor::Stream(ref p) => return unsafe { (*p.get()).send(t).map_err(SendError) },
            Flavor::Shared(ref p) => return unsafe { (*p.get()).send(t).map_err(SendError) },
        ...
        let (packet, sleeper, guard) = match *unsafe { self.inner() } {
            Flavor::Oneshot(ref p) => {
                ...
                unsafe {
                    ...
                    match (*p.get()).upgrade(rx) {
                        oneshot::UpSuccess | oneshot::UpDisconnected => (a, None, guard),
            ...
            Flavor::Stream(ref p) => {
                ...
                unsafe {
                    ...
                    let rx = Receiver::new(Flavor::Shared(a.clone()));
                    match (*p.get()).upgrade(rx) {
                        stream::UpSuccess | stream::UpDisconnected => (a, None, guard),
...
impl<T> Drop for Sender<T> {
    fn drop(&mut self) {
        match *unsafe { self.inner_mut() } {
            Flavor::Oneshot(ref mut p) => unsafe {
                (*p.get()).drop_chan();
            },
            Flavor::Stream(ref mut p) => unsafe {
                (*p.get()).drop_chan();
            },
            Flavor::Shared(ref mut p) => unsafe {
                (*p.get()).drop_chan();
            },
            Flavor::Sync(..) => unreachable!(),
        }
    ...
    pub fn recv(&self) -> Result<T, RecvError> {
        loop {
            let new_port = match *unsafe { self.inner() } {
                ...
                Flavor::Sync(ref p) => return unsafe { (*p.get()).recv().map_err(|_| RecvError) },
            };
            unsafe {
                mem::swap(self.inner_mut(), new_port.inner_mut());
            }
    ...
    fn abort_selection(&self) -> bool {
        ...
        loop {
            let result = match *unsafe { self.inner() } {
                ...
                Flavor::Stream(ref p) => unsafe { (*p.get()).abort_selection(was_upgrade) },
                Flavor::Shared(ref p) => return unsafe { (*p.get()).abort_selection(was_upgrade) },
                Flavor::Sync(ref p) => return unsafe { (*p.get()).abort_selection() },
            };
