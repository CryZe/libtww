...
use std::sync::mpsc::blocking::{self, SignalToken};
use std::sync::mpsc::mpsc_queue as mpsc;
use std::sync::mpsc::select::StartResult::*;
use std::sync::mpsc::select::StartResult;
...
impl<T> Packet<T> {
    ...
    pub fn inherit_blocker(&mut self, token: Option<SignalToken>, guard: MutexGuard<()>) {
        token.map(|token| {
