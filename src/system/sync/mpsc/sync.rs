...
use std::sync::mpsc::blocking::{self, WaitToken, SignalToken};
use std::sync::mpsc::select::StartResult::{self, Installed, Abort};
...
impl<T> Packet<T> {
    ...
    pub fn drop_port(&self) {
        ...
        let mut queue = mem::replace(&mut guard.queue,
                                     Queue {
                                         head: ptr::null_mut(),
                                         tail: ptr::null_mut(),
                                     });
